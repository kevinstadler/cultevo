...
mantel.plotfuns[["r"]] <- function(mantels, ylim=range(0.1, mantels$veridical, mantels$mean+mantels$sd, mantels$mean-mantels$sd)+c(-0.1,0.1), xlab="", ...) {
  errbar(1:nrow(mantels), mantels$mean, mantels$mean+mantels$sd, mantels$mean-mantels$sd, ylab="r (mean±sd)", ylim=ylim, errbar.col=red.if.na(mantels$p.smoothed), ...)
...
#    d[[i]] <- density(unlist(mantels$msample[i]))
    d[[i]] <- hist(unlist(mantels$msample[i]), breaks=nbins)
    maxdensity <- max(maxdensity, d[[i]]$density, dnorm(0, sd=mantels$sd[i]))
  }
  for (i in 1:nrow(mantels)) {
    xlim <- range(mantels$msample[i], mantels$veridical[i])
    plot(d[[i]], freq=FALSE, yaxs="i", xlim=xlim, ylim=c(0, maxdensity), xlab="r", ylab="Density", border="gray", main=main, ...)
    ...
    text(mantels$veridical[i], c(level+par("cxy")[2], level), paste(c("r", "z"), round(c(mantels$veridical[i], mantels$z[i]), digits=2), sep="="), pos=3)
  }
}
